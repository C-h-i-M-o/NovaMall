...
业务范围只保留数据库技术需要的最小订单链路：会员地址、购物车、跨店结算、模拟支付、取消、订单快照、有效销量和审计查询。不实现完整电商订单中心、复杂售后、真实支付、物流、优惠、评价或运营报表。

## 2. 成功标准

- 从 Stage 3 数据库继续执行迁移，新增最小订单域表和只读视图；
- `sp_checkout_cart` 存储过程具备输入参数、输出参数、事务、库存行锁、跨店拆单、订单快照、购物车清理和 checkoutToken 幂等；
- 库存不足、商品不可售、地址越权和空购物车时，结算事务完整回滚；
- 模拟支付、取消待支付订单和库存恢复使用短事务完成；
- 总订单、子订单、角色授予和商品变更写入脱敏审计日志；
- 会员订单明细、有效销量和店铺销售汇总通过视图提供；
- Top 10 使用窗口函数并有稳定并列排序；
- 分类商品、店主商品、会员订单、店铺订单、审计日志和全文检索索引均有测试或证据；
- AES 手机号加密继续使用每条记录独立 IV，并补齐订单快照密文验证；
- `docs/evidence/database/stage-4-core.md` 下用精简表格保存 9 项数据库技术的真实执行证据。
...
5. 校验商品 `PUBLISHED`、分类 `ACTIVE`、店铺 `ACTIVE`、数量大于 0、库存充足。
6. 使用数据库当前 `products.price` 计算金额，不信任客户端金额。
7. 创建总订单，按店铺创建子订单，创建订单明细快照。
8. 扣减库存并增加商品版本号。
...
| `PRODUCT_UNAVAILABLE` | 商品、分类或店铺不可售 | `PRODUCT_UNAVAILABLE` |
...
按店铺和日期聚合有效订单数、商品数量和销售额，供 Stage 5 最小店主概览使用。
...
| 方法 | 路径 | 角色 | 行为 |
|---|---|---|---|
| GET | `/owner/shop-orders` | OWNER | 本店子订单列表 |
...
| PRODUCT_UNAVAILABLE | 409 | 商品、分类或店铺不可售 |
...
该文件用一张表覆盖存储过程、触发器、视图、索引优化、事务并发、窗口函数、审计日志、全文检索和 AES 加密。Stage 3 已生成的全文检索原始记录继续保留在 `docs/evidence/database/catalog-search.md`，不重复拆分。性能数字和执行计划必须来自真实命令，不能预填。
