...
跨店购物车结算需要同时完成库存锁定、总订单创建、按店铺拆单、订单快照、库存扣减、支付记录创建和购物车清理。
...
按店铺和日期汇总有效订单数、商品数量和销售额，供店主经营概览使用。
...
| shops | UQ(owner_user_id), UQ(name) | 一人一店、店名唯一 |
| products | (category_id, status, id) | 分类商品分页 |
| products | (shop_id, status, updated_at, id) | 店主商品管理 |
...
| shop_orders | (shop_id, status, updated_at, id) | 店铺订单分页 |
...
FROM (
  SELECT
    ...
    sales_amount,
    ROW_NUMBER() OVER (
      ORDER BY sold_quantity DESC, product_id ASC
    ) AS sales_rank
...
显式次级排序保证销量相同时结果稳定。MySQL 窗口函数语义参考：[Window Functions](https://dev.mysql.com/doc/refman/8.4/en/window-functions.html)。
